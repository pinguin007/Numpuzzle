...
int main(void) {
  ...
  while(1){
    ...
    while(j < (size-1)){  // randomizing the location of the numbers  except the last spot. that's already a zero
          int flag=0;
    ...
    while(playerEntry != 0){ //main game loop. only broken when the player enters 0, or wins by putting the random table in order
      ...
      //if the playerEntry is one of those cells, the playerEntry will be moved to the empty cell location
      ...
      if(playerEntry==75949){     //cheat to solve the gameBoard
        for(int i=1; i < size; i++){
          gameBoard[i-1]=i;
        }
      }
      
      while(1){  //checks if the board is organized by counting from the beginning and checking if the next number is greater by a factor of 1.
        int counter1=0;
        for(int i=0; i<size;i++){
